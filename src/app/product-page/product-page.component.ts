...
categories = [
  ...
  selectedMinPrice: number = this.priceRange.min;
    ...
    .subscribe({
      next: (response) => {
        if (response.status && response.data) {
          // ✅ Keep only apparel categories (1,2,3)
          this.products = response.data.filter((p: ProductWithSize) =>
            [1, 2, 3].includes(p.categoryId)
          );
...
getSelectedSize(productId: number): number | undefined {
  return this.productSelectedSizes.get(productId);
}
